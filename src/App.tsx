...
import { getAppColorClasses } from './lib/types';
...
export default function App() {
  ...
  const howToSteps = [
    { num: '01', title: 'Escolhe a app',      desc: 'Seleciona a aplicacao VPN que tens instalada no teu telemovel.' },
    { num: '02', title: 'Escolhe o ficheiro', desc: 'Ves ate 5 ficheiros. Escolhe e carrega em Download.' },
    { num: '03', title: 'Importa na app',     desc: 'Abre a tua app VPN, vai a Importar e seleciona o ficheiro.' },
    { num: '04', title: 'Usa a password',     desc: 'Se tiver password, carrega no botao PASS para ver e copiar.' },
  ];
  ...
  return (
    <div className="min-h-screen bg-[#0a0e14] text-white font-sans">
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="max-w-6xl mx-auto px-4 py-12 text-center relative">
          ...
          <p className="text-gray-400 text-sm max-w-md mx-auto mb-8">
            Ficheiros prontos para importar nas tuas apps VPN favoritas.<br />
          ...
          <div className="inline-flex items-center gap-6 bg-[#111820] rounded-2xl border border-white/5 px-8 py-4">
            <div className="text-center">
              ...
              <div className="text-[9px] text-gray-500 uppercase tracking-widest font-bold">Apps VPN</div>
            </div>
            <div className="w-px h-8 bg-white/10" />
            <div className="text-center">
              <div className="text-2xl font-black text-cyan-400">{rows.length}</div>
              <div className="text-[9px] text-gray-500 uppercase tracking-widest font-bold">Ficheiros</div>
            </div>
            <div className="w-px h-8 bg-white/10" />
            <div className="text-center">
              <div className="text-2xl font-black text-cyan-400">0 KZ</div>
              <div className="text-[9px] text-gray-500 uppercase tracking-widest font-bold">Custo</div>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {apps.map((app, i) => {
            ...
            return (
              ...
              >
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center mb-4 border text-xl ${colors.card}`}>
                  {app.icon}
                </div>
                <h3 className="font-bold text-lg mb-2">{app.name}</h3>
                <p className="text-gray-500 text-xs leading-relaxed mb-4">{app.description}</p>
                <div className="flex items-center justify-between">
                  <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full ${colors.badge}`}>
                    {counts[app.id] ?? 0} ficheiros disponiveis
                  </span>
                  <button
      ...
      {/* How To Use */}
      <section className="max-w-6xl mx-auto px-4 py-12">
        <h2 className="text-[10px] font-bold text-gray-500 tracking-[0.2em] uppercase mb-6">COMO USAR</h2>
        ...
        <div className="space-y-4">
          {comments.length === 0 ? (
            ...
            comments.map((comment) => (
              <div key={comment.id} className="bg-[#111820] border border-white/5 rounded-2xl p-5">
